...
public class BulkEmailComponent {
	public static void main(String[] args) throws Exception {
		BulkEmailComponent bc=new BulkEmailComponent();
		bc.EmailService();
	}
	public void EmailService() throws Exception {
    	...
    	System.out.println("Please Enter Sender's G-mail ID :  ");SenderID=scan.next();
    	System.out.println("Please Enter Sender's G-mail Password :  ");SenderPassword=scan.next();
    	System.out.println("Please Enter Subject of E-mail :  ");Subject=scan.next();
    	System.out.println("Please Enter Message Content :  ");MessageContent=scan.next();
    	System.out.println("Please Enter the number of attachment  ");
    	int NosAttach=scan.nextInt();
    	if(NosAttach!=0) {
    		for (int i = 0; i < NosAttach; i++) {
    			System.out.println("Please path of Attachment"+(i+1)+" :  ");String path=scan.next();
    			AttachmentPath.add(path);
			}
    	}else{
				AttachmentPath=null;
			}
    	System.out.println("Please Enter the number of Receivers  ");int NosReceivers=scan.nextInt();
    	if(NosReceivers!=0 && NosReceivers>0) {
    		for (int i = 0; i < NosReceivers; i++) {
    			System.out.println("Please enter Email ID of Receiver "+(i+1)+" :  ");String IDs=scan.next();
    			ReceiverIDs.add(IDs);
			}
    	}else{
